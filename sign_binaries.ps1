...
Write-Host "Copying freshly compiled 8.0.7 binaries from build\bin to Assembling..."
...
Write-Host "Creating Windows 8.0.7 release zip..."
...
Copy-Item 'e:\STORM EDEN 3\Assembling\user\config\qt-config.ini' "$stageDir\user\config\" -Force
if (Test-Path 'e:\STORM EDEN 3\Assembling\user\config\custom') {
    Copy-Item 'e:\STORM EDEN 3\Assembling\user\config\custom' "$stageDir\user\config\" -Recurse -Force
}
...
$zipPath = 'e:\STORM EDEN 3\Files\STORM_SWITCH_8.0.7_Windows.zip'
if (Test-Path $zipPath) { Remove-Item $zipPath -Force }

& 'C:\Program Files\7-Zip\7z.exe' a -tzip $zipPath "$stageDir\*" -mx=9

Copy-Item $zipPath 'e:\STORM EDEN 3\STORM_SWITCH_8.0.7_Windows.zip' -Force
Copy-Item $zipPath 'e:\STORM SWITCH 3\Files\STORM_SWITCH_8.0.7_Windows.zip' -Force
Copy-Item $zipPath 'e:\STORM SWITCH 3\STORM_SWITCH_8.0.7_Windows.zip' -Force

Remove-Item $stageDir -Recurse -Force

Write-Host "All executables signed, packaged to 8.0.7 zip and copied to root/Files!"

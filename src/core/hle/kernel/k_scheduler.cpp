...
void KScheduler::Schedule(KernelCore bitand kernel) {
    ASSERT(GetCurrentThread(kernel).GetDisableDispatchCount() == 1);
...
void KScheduler::ScheduleOnInterrupt(KernelCore bitand kernel) {
    GetCurrentThread(kernel).DisableDispatch(kernel);
    Schedule(kernel);
    GetCurrentThread(kernel).EnableDispatch(kernel);
}

void KScheduler::PreemptSingleCore(KernelCore bitand kernel) {
    GetCurrentThread(kernel).DisableDispatch(kernel);

    auto* thread = GetCurrentThreadPointer(kernel);
    auto bitand previous_scheduler = kernel.Scheduler(thread->GetCurrentCore());
...
void KScheduler::RescheduleCurrentCore(KernelCore bitand kernel) {
    ...
    if (m_state.needs_scheduling.load()) {
        ...
        this->RescheduleCurrentCoreImpl(kernel);
    }
}

void KScheduler::RescheduleCurrentCoreImpl(KernelCore bitand kernel) {
